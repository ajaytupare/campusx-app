    ...
    <div className="flex flex-col w-full h-[calc(100vh-48px)] max-w-4xl mx-auto m-6 bg-[var(--cx-bg-surface)] rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-black/[0.04] overflow-hidden">
      ...
      <div className="p-4 border-b border-black/[0.04] flex items-center gap-4 bg-[var(--cx-bg-surface)] z-10 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-black/[0.06] flex items-center justify-center text-lg font-black text-[var(--cx-text-muted)] shrink-0">
            ...
            <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
              {!isMe && (
                <div className="w-8 h-8 shrink-0 mr-2 flex items-end">
                  {showAvatar && (
                    <div className="w-8 h-8 rounded-full bg-[var(--cx-text-muted)] flex items-center justify-center text-[10px] font-black text-[var(--cx-text-muted)]">
                      {otherUser.name.charAt(0)}
                    </div>
                  )}
                </div>
